...
class WeatherApiServices {
  ...
  Future<DirectGeocoding> getDiractGeoCoding(String city) async {
    final Uri uri = Uri(
        scheme: 'https',
        host: kApiHost,
        path: '/geo/1.0/direct',
  ...
  Future<Weather> getWeather(DirectGeocoding diractGeocoding) async {
    final Uri uri = Uri(
        scheme: 'https',
        host: kApiHost,
        path: '/data/2.5/weather',
        queryParameters: {
          'lat': '${diractGeocoding.lat}',
          'lon': '${diractGeocoding.lon}',
          'units': kUnit,
          'appid': dotenv.env['APPID']
        });
    try {
